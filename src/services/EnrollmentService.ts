...
import { MlRiskService } from "./MlRiskService"
...
export class EnrollmentService {
    ...
    static async create(data: unknown) {
        ...
        const risk = await MlRiskService.predictByEnrollment(enrollment.id)
    ...
    static async getRisk(enrollmentId: string) {
        ...
        const risk = await MlRiskService.predictByEnrollment(enrollmentId)

        if (!risk) {
            throw new AppError("Não foi possível calcular o risco.", 502)
        }
